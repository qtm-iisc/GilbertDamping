...
def fourier_q_to_R(AA_q, mp_grid, kpt_mp_grid, iRvec, ndegen, numthreads=1, fft='fftw'):
    ...
    #print(AA_q.shape)
    shapeA = AA_q.shape[1:] 
    #print(shapeA)# remember the shapes after q
    ...
    #print(AA_q_mp[tuple(np.array([0,0,0]))])
...
def fourier_R_to_q(AA_R1, mp_grid1, kpt_mp_grid1, iRvec1, numthreads=1, fft='fftw'):
    print_my_name_start()
    mp_grid1 = tuple(mp_grid1)
    #print(mp_grid1)
    shapeA = AA_R1.shape[1:]  # remember the shapes after q
    #print(shapeA)
    AA_R_mp = np.zeros(tuple(mp_grid1) + shapeA, dtype=complex)
    #print(AA_R_mp.shape)
    for i, iR in enumerate(iRvec1):
        AA_R_mp[tuple(iR % mp_grid1)] = AA_R1[i]
    #print("FFT START")
    AA_k = FFT(AA_R_mp, axes=(0, 1, 2),inverse=True, numthreads=numthreads, fft=fft, destroy=False)
    AA_k = AA_k*np.prod(mp_grid1)
    AA_kf = np.array([AA_k[tuple(ik)]  for ik in kpt_mp_grid1])
    #eigval,eigvec = LA.eigh(AA_k[tuple(np.array([0,0,0]))])
    #print(eigval)
    #print("FFT END")
    print_my_name_end()
    return AA_kf
