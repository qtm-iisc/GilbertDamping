...
class CheckPoint():
    ...
    def __init__(self, seedname):
        ...
        #print('Reading restart information from file ' + seedname )
...
###### hamiltonian #######
    def get_HH_q(self, eig):
        ...
        #for ik, E in enumerate(eig.data):
        #    print(ik,E)
...
###### Green's function #######
### eta is spectral width ###
#### e_val is the energy value at which we want ####
### primirily we need at fermi level ###
    def get_GG_q(self, eig, eta, e_val):
        assert (eig.NK, eig.NB) == (self.num_kpts, self.num_bands)
        #for ik, E in enumerate(eig.data):
        #    print(ik,E)
        GG_q = np.array([self.wannier_gauge(1/(e_val+1j*eta-E), ik, ik) for ik, E in enumerate(eig.data)])
        #print(GG_q[0])
        return GG_q 
...
class SPN(W90_data):
    ...
    def __init__(self,seedname, formatted=False):
        #print("----------\n SPN  \n---------\n")
        if formatted:
            f_spn_in = open(seedname , 'r')
            SPNheader = f_spn_in.readline().strip()
            nbnd, NK = (int(x) for x in f_spn_in.readline().split())
        else:
            f_spn_in = FortranFileR(seedname)
        ...
        #print("reading {} : {}".format(seedname, SPNheader))
